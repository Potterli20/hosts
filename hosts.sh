...
# Update index.html stats with formatted numbers
perl -i -pe '
    my ($pro, $lite, $allow_cnt, $yt, $cn, $gfw) = (
        qx(wc -l < ad-hosts-pro) =~ /(\d+)/,
        qx(wc -l < ad-hosts-sqlist) =~ /(\d+)/,
        qx(wc -l < allow) =~ /(\d+)/,
        qx(wc -l < ad-youtube-hosts) =~ /(\d+)/,
        qx(wc -l < dns-hosts-china) =~ /(\d+)/,
        qx(wc -l < dns-hosts-gfw) =~ /(\d+)/
    );
    sub fmt { $_ = shift; 1 while s/(\d+)(\d{3})/$1,$2/; $_ }
    s{>Ad-hosts-pro: <strong>[^<]*</strong>}{">Ad-hosts-pro: <strong>".fmt($pro)."</strong>"}e;
    s{>Ad-hosts-lite: <strong>[^<]*</strong>}{">Ad-hosts-lite: <strong>".fmt($lite)."</strong>"}e;
    s{>Allowlist: <strong>[^<]*</strong>}{">Allowlist: <strong>".fmt($allow_cnt)."</strong>"}e;
    s{>DNS国内分流加速规则: <strong>[^<]*</strong>}{">DNS国内分流加速规则: <strong>".fmt($cn)."</strong>"}e;
    s{>DNS国外分流加速规则: <strong>[^<]*</strong>}{">DNS国外分流加速规则: <strong>".fmt($gfw)."</strong>"}e;
' index.html
